...
1. **No new spending.** No paid services, no signups requiring payment info, no domain purchases. Domain availability gets checked, never bought.
2. **Publish nothing.** Everything stays local or repo-shaped for github.com/Obviouslyobvi/Insite. No deployment, no posting, no messaging real people.
3. **Invent nothing.** Every quote, stat, competitor fact, and market claim traces to a real fetched URL. Inferences are labeled inference. Unverified items are labeled unverified. A smaller thesis on real evidence beats a grand one on plausible fiction.
4. **Work inside projects/insite.** All artifacts land there.
5. **No mid-run questions.** Blocked is not an option. If a phase stalls, ship the strong 80 percent, note the cut, keep moving. Questions, working answers, and reasoning go to the Build Log for one batch submission at the end.
...
| 5 | Marketing one-pagers: Developer, City/Agency, Capital Partner | 03_MARKETING/ | On-brand, accurate, disclaimered |
...
| 10 | Red-team Kill Memo | 08_REDTEAM/ | Honest attempt to kill the business; survivors and casualties listed |
...
Fetched, verified, and logged in the Claims Register (01_EVIDENCE/Claims_Register.md) with URLs:
...
- SB 684 (2023) and SB 1123 (2024): ministerial approval for subdivisions of 10 or fewer lots/units; the demand-side tailwind. VERIFIED.
...
- Terner Center fee studies: 2015 average $23,455 per single-family unit, about 3x national average; total charges can exceed $150K per unit; 2020-2023 LIHTC average about $19,806 per unit. VERIFIED.
...
- **Fan-out research:** parallel source hunts per claim category (statutes, competitors, fees, permits, rates, housekeeping).
...
Plan (this document, delivered first) -> Claims Register -> Whitepaper -> INS-201 Checklist -> Brand kit -> One-pagers -> Website -> Deck -> Build Log with questions -> Kill Memo -> Final QC -> Package handoff with repo-ready file tree.

## 10. Risks to this run and fallbacks

- **Tool or fetch failure:** switch route (alternate mirror, direct file download, different library). Already exercised: Census county data pulled from the raw data server when the page scrape failed.
...
*This plan is the contract for the run. Deviations are recorded in the Build Log.*
